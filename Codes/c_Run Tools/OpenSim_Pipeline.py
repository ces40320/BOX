...
class OpenSimPipeline:
    ...
    def __init__(self, path_base=None, root_dir=None, sub_name=None, APP=None,
        ...
        if path_base is not None:
            ...
            path_class = path_class or AsymmetricOneCyclePath
...
if __name__ == "__main__":
    ...
    try:
        ...
        CONFIG = {"root_dir": r"E:\Dropbox\SEL\BOX\OpenSim\_Main_\c_AddBio_Continous", "sub_name": "SUB1", "APP": "APP2_OneCycle", "symmetric": True}
        def get_path_from_config(cfg): from PipelinePathSetting import AsymmetricOneCyclePath; return AsymmetricOneCyclePath.from_config(cfg)
        def update_config(**kw): CONFIG.update(kw)

    update_config(sub_name="SUB2", symmetric=False, task_type="OneCycle")
    path_base = get_path_from_config(CONFIG)
    pipeline = OpenSimPipeline(path_base=path_base)
    kg_bpm = CONFIG.get("kg_bpm", ["15_10", "15_16"])
    trial_range = (1, 3)
    task_range = (1, 11)
