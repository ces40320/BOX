...
> **작성일**: 2026-04-07  
> **최종 갱신**: 2026-04-08 — 인프라 정비 완료 (`PATH_RULE.py` 클래스화, `config_methods.py` 구조 정비, `config_exp_settings.py` 신규, `run_get_exp_data.py` 분기 구현)
...
- [ ] 파이프라인 구현 시 실제 호출부에서 `rotations=None` 적용 (현재 파이프라인은 stub)
...
- [ ] `lifting_io.py`에 `assemble_app3()` 함수 추가 (AddBox: 손가락 마커 COP)
- [ ] Symmetric 프로토콜용 세그먼트 분할 함수 구현 (`segment_symmetric.py`)
...
| 순서 | 작업 | 관련 문제 | 난이도 | 상태 |
|------|------|----------|--------|------|
| 1 | ~~실행 파일에서 `rotations=None` 지정~~ | 문제3 | 즉시 | ✅ config 완료, 호출부는 파이프라인 구현 시 |
...
| 6 | `assemble_app3()` 함수 추가 (AddBox: 손가락 마커 COP) | 5.4-A | 중간 | ⬜ |
| 7 | Symmetric 세그먼트 분할 구현 (`segment_symmetric.py`) | 5.4-A | 중간~큰 | ⬜ |
| 8 | ~~프로토콜 분기 진입점 구현~~ | 5.4-C | 중간 | ✅ `run_get_exp_data.py` 완료 |
