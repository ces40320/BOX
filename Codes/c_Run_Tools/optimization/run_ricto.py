...
def run_legacy_task(
    ...
    dirs = _path.legacy_onecycle_dirs(sub=sub, trial_folder=trial_folder)
    ...
    written = []
    for mode in modes:
        out_mot = _path.legacy_extload_mot_path(
            kg_bpm=kg_bpm,
            trial_num=trial_num,
            task_num=task_num,
            mode=mode,
...
def run_modern_segment(
    *,
    ...
    rp = _path.ResultPaths(sub_label, protocol=protocol)
...
def main() -> None:
    ...
    ap.add_argument("--namecode", default=None, help="Modern: SUB label e.g. SUB2")
    ap.add_argument("--protocol", default="Asymmetric")
