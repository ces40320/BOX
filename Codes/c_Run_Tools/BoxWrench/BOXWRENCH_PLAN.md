...
| Free-box IK + BK + States | `boxwrench_opensim.py` (vendor Load* pattern) |
...
- Free-box outputs (written by BoxWrench):
  ...
  - ExtLoad: `…/AB/ExtLoad/SUB7_7kg_10bpm_1AB_ExtLoad_BoxWrench.mot`

### RiCTO / Analysis

- PATH_RULE: `Analysis/Asymmetric/RiCTO/TimeSeries/SUB7/7kg_10bpm/SUB7_7kg_10bpm_1AB_RiCTO_timeseries.csv`
- BoxWrench forces CSV: `Analysis/Asymmetric/BoxWrench/TimeSeries/SUB7/7kg_10bpm/SUB7_7kg_10bpm_1AB_BoxWrench_forces.csv`
...
BoxWrench OpenSim allocation uses the **CAD body** row (`HANDLE_L_NOM` / `HANDLE_R_NOM`).
...
3. **Box COM wrench** — COM accel + ω̇ → `F`, `M` (`boxwrench_kinematics`).
4. **L/R allocation** — SLSQP at CAD handle centers (`boxwrench_allocate`).
5. **RiCTO gate** — multiply hand forces by smooth/rect weight during handle contact window.
6. **ExtLoad** — HeavyHand GRF template; hand3=L, hand4=R (`boxwrench_extload`).
...
BoxWrench/
├─ BOXWRENCH_PLAN.md
├─ run_boxwrench.py              # CLI; --synthetic | --namecode 260526_PJH
├─ build_box_model_with_markers.py
├─ boxwrench_config.py           # SAMPLE_* defaults = 260526_PJH
├─ boxwrench_paths.py            # Dropbox vs local OpenSim_Process resolve
├─ boxwrench_opensim.py          # free-box IK / BK / States (no vendor import)
├─ boxwrench_markers.py
├─ boxwrench_rotation.py
...
C:\Users\ok\anaconda3\envs\osim\python.exe Codes\c_Run_Tools\BoxWrench\run_boxwrench.py --namecode 260526_PJH

:: Optional overrides
…\python.exe …\run_boxwrench.py --namecode 260526_PJH --condition 7kg_10bpm --segment 1AB --force-kinematics

:: Synthetic (no OpenSim)
…\python.exe …\run_boxwrench.py --synthetic --box-mass 7
```
...
```bat
C:\Users\ok\anaconda3\envs\osim\python.exe Codes\c_Run_Tools\BoxWrench\run_boxwrench.py --namecode 260526_PJH --condition 7kg_10bpm --segment 1AB --force-kinematics
```
...
| ExtLoad BoxWrench | `…/ExtLoad/SUB7_7kg_10bpm_1AB_ExtLoad_BoxWrench.mot` |
| Analysis CSV | `Analysis/Asymmetric/BoxWrench/TimeSeries/SUB7/7kg_10bpm/…_forces.csv` |
...
2. Optional: add `BoxWrench` to default protocol `APPs` once MOT generation is routine.
