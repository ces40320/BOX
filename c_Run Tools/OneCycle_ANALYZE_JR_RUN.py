...
def JR_RUN(root_dir, sub_name, APP, kg_bpm, trial_num, task_num):
...
def JR_RUN_g(root_dir, sub_name, APP, kg_bpm, trial_num, task_num):       # APP4 전용: EHF 추정값 획득
    model_path = root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ sub_name +'_Scaled_'+ APP[0:4] +'_Actuator.osim'
    model = osim.Model(model_path)
...
sub_name = 'SUB1'
APP = ['APP1_OneCycle','APP2_OneCycle']
kg_bpm = ['15_10','15_16']

for condition in range(0,2-1):
    for EHF in range(0,2):
        for trial_num in range(1,3):
            for task_num in range(1,11):
                start_time=time.time()
                
                JR_RUN(root_dir, sub_name, APP[EHF], kg_bpm[condition], str(trial_num), str(task_num))
                if APP[EHF] == 'APP4':
                    JR_RUN_g(root_dir, sub_name, APP[EHF], kg_bpm[condition], str(trial_num), str(task_num))    # APP4 전용
                
                end_time=time.time()
                # 실행 시간 계산 및 출력
                execution_time = end_time - start_time
                print(f"{APP[EHF]} {kg_bpm[condition]}_{str(trial_num)}_{str(task_num)} JR 코드 실행 시간: {execution_time} 초")
