...
def JR_SET(root_dir, sub_name, APP, kg_bpm, trial_num, UpDown, task_num):
    trc_file_path = (root_dir+'\\'+sub_name+'\\'+'UpDown_TrcMot'+'\\'
    ...
    analyze.setName(sub_name+'_'+ kg_bpm +'_'+ str(trial_num) +'_'+ UpDown + str(task_num) +'_'+ APP)
    ...
    analyze.setControlsFileName(root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ 'trial'+ kg_bpm +'_'+ str(trial_num) +'\\SO_Results'+'\\'
                                + sub_name +'_'+ kg_bpm +'_'+ str(trial_num) +'_'+ UpDown + str(task_num) +'_'+ APP +'_StaticOptimization_activation.sto')
    ...
    if APP == 'APP4':   # 'APP4'일 때는 grf_file_path, IK 에 있는 APP변수를 'APP3'로 변경 후 실행해야 함.
        grf_file_path = (root_dir +'\\'+ sub_name +'\\'+ 'APP3' +'\\'+ 'trial'+ kg_bpm +'_'+ str(trial_num) +'\\xml_ExtLoad\\'
                        +'SETUP_ExtLoad_'+ kg_bpm +'_trial'+ str(trial_num) +'_'+ UpDown + str(task_num) +'_'+ 'APP3' +'.xml')
    else:
    ...
    jr.setForcesFileName(root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ 'trial'+ kg_bpm +'_'+ str(trial_num) +'\\SO_Results'+'\\'
                         + sub_name +'_'+ kg_bpm +'_'+ str(trial_num) +'_'+ UpDown + str(task_num) +'_'+ APP +'_StaticOptimization_force.sto')
...
def JR_XML_SET(root_dir, sub_name, APP, kg_bpm, trial_num, UpDown, task_num):
    ...
    for trial_num in range(1,3-1):
        for ud in range(0+1,2):
            for task_num in range(1,11):
                JR_SET(root_dir, sub_name, APP, kg_bpm[condition], str(trial_num), UpDown[ud], str(task_num))

for condition in range(0,2-1):
    for trial_num in range(1,3-1):
        for ud in range(0+1,2):
            for task_num in range(1,11):
                JR_XML_SET(root_dir, sub_name, APP, kg_bpm[condition], str(trial_num), UpDown[ud], str(task_num))
