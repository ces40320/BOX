...
def SO_RUN(root_dir, sub_name, APP, kg_bpm, trial_num, UpDown, task_num):
    model_path = root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ sub_name +'_Scaled_AddBiomech_'+ APP +'.osim'
    ...
    model.printToXML(root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ sub_name +'_Scaled_'+ APP +'_Actuator.osim')
    
    
    so = osim.AnalyzeTool(root_dir +'\\'+ sub_name +'\\'+ APP +'\\'+ 'trial'+ kg_bpm +'_'+ str(trial_num) +'\\SO_Results\\'
                         +'SETUP_SO_' +kg_bpm +'_trial'+ str(trial_num) +'_'+ UpDown + str(task_num) +'_'+ APP +'.xml')
    so.setModel(model)
...
sub_name = 'SUB2'
APP = ['APP1','APP2','APP4','APP3']
kg_bpm = ['15_10_S','15_16']
UpDown = ['U','D']

for condition in range(0,2-1):
    for EHF in range(0,4-1):
        for trial_num in range(1,3-1):
            for ud in range(0+1,2):
                for task_num in range(1,11):
                    start_time=time.time()
                    SO_RUN(root_dir, sub_name, APP[EHF], kg_bpm[condition], str(trial_num), UpDown[ud], str(task_num))
                    end_time=time.time()
                    # 실행 시간 계산 및 출력
                    execution_time = end_time - start_time
                    print(f"{APP[EHF]} {kg_bpm[condition]}_{str(trial_num)}_{UpDown[ud]}{str(task_num)} SO 코드 실행 시간: {execution_time} 초")
